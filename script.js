const SITE = {
  ...
  url: typeof SITE_URL !== "undefined" ? SITE_URL : "",
};

/** GitHub Pages: fix profile photo + styles path */
...
function fixGitHubPagesAssets() {
  const base = getGitHubPagesBase();
  if (!base) return;

  const photo = `${base}profile.png`;
  document.querySelectorAll('img[src*="profile"]').forEach((img) => {
    img.src = photo;
  });

  document.querySelectorAll('link[href="styles.css"], link[href^="styles.css"]').forEach((link) => {
    const href = link.getAttribute("href");
    if (href && !link.href.startsWith(base)) {
...
if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", fixGitHubPagesAssets);
} else {
  fixGitHubPagesAssets();
}
